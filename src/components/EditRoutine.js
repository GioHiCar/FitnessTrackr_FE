...
const EditRoutine = () => {
    ...
    const handleSubmit = async (event) => {
      ...
      const exists = myInfo.find(function(info, index) {
        ...
        if (event.target[0].value === "" || event.target[1].value === "") {
          ...
          const patched = await patchRoutines(name, goal,isPublic, token,routineId)
  ...
  {singleRoutine ? <form onSubmit={handleSubmit}>
    <label className="my_routine">
      Name:
      <input placeholder="Enter Routine Name" id="name" defaultValue={singleRoutine.name} />
    </label>

    <label className="my_routine">
      Goal:
      <input placeholder="Enter Routine Goal" id="goal" defaultValue={singleRoutine.goal} />
    </label>
    <fieldset>
      <legend>
        Keep Routine ::
